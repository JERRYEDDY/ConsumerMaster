...
{
    public partial class CompositeProcedureCodes : Page
    {
        ...
        protected void RadGrid1_NeedDataSource(object source, GridNeedDataSourceEventArgs e)
        {
            ...
            string selectQuery =
                @"  SELECT 
	                    cpc.id AS cpc_id, cpc.name AS cpc_name,tp.name AS trading_partner_name,cpc.taxonomy_code AS cpc_taxonomy_code
                    FROM 
                        CompositeProcedureCodes AS cpc 
                    INNER JOIN 
                        TradingPartners AS tp ON trading_partner_id = tp.id
                "; 
